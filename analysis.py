...
def plotSFH(galaxy):
	...
	bins = SFH[nameMask, 0, :][0] 
	weights = SFH[nameMask, 1, :][0]
...
def plotCEH(galaxy):
	...
	plt.figure(figsize=(10,8))
	nameMask = singleNames == galaxy
	ageBins = CEH[nameMask, 0, :, :][0]
	metalBins = CEH[nameMask, 1, :, :][0]
	weights = CEH[nameMask, 2, :, :][0]
...
singleStellarMass = galaxies['stellar_mass'] # one per galaxy
names = summary['name']
...
attEnergy = summary['attenuated_energy']
emitEnergy = summary['emitted_energy']
SFH = galaxies['sfh']
CEH = galaxies['ceh'] 
...
(dp_flux_og, dp_err_og, dp_bool_og, dp_axisRatio_og, dp_stellarMass_og, dp_dustMass_og, dp_SFR_og, 
													dp_diskMask_og, dp_faceOnMask_og) = dustPedia()
...
dpSFRMask_og = (dp_SFR_og[dpMassMask_og]/dp_stellarMass_og[dpMassMask_og]
			    >= np.amin(sSFR)) & (dp_SFR_og[dpMassMask_og]/dp_stellarMass_og[dpMassMask_og] <= np.amax(sSFR))
...
dpMassMask = (dp_stellarMass_og >= np.amin(stellarMass[sphMassMask])) & (dp_stellarMass_og <= 
			  np.amax(stellarMass[sphMassMask]))
dpSFRMask = (dp_SFR_og[dpMassMask]/dp_stellarMass_og[dpMassMask] >= 
	 		 np.amin(sSFR[sphMassMask])) & (dp_SFR_og[dpMassMask]/dp_stellarMass_og[dpMassMask] <= 
	 		 np.amax(sSFR[sphMassMask]))
dpSFRMask_separate = (dp_SFR_og/dp_stellarMass_og >= np.amin(sSFR[sphMassMask])) & (dp_SFR_og/dp_stellarMass_og <= 
					  np.amax(sSFR[sphMassMask]))
dp_diskMask = dp_diskMask_og[dpMassMask][dpSFRMask]
dp_faceOnMask = dp_faceOnMask_og[dpMassMask][dpSFRMask][dp_diskMask]
dp_flux, dp_err, dp_bool, dp_axisRatio, dp_stellarMass, dp_dustMass, dp_SFR = (dp_flux_og[dpMassMask][dpSFRMask], 
																			  dp_err_og[dpMassMask][dpSFRMask], 
																			  dp_bool_og[dpMassMask][dpSFRMask], 
																			  dp_axisRatio_og[dpMassMask][dpSFRMask], 
																			  dp_stellarMass_og[dpMassMask][dpSFRMask], 
																			  dp_dustMass_og[dpMassMask][dpSFRMask], 
																			  dp_SFR_og[dpMassMask][dpSFRMask])
